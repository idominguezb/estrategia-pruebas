...
console.log(inputValues);
  ...
  (async () => {
    ...
    for(b of browsers){
      ...
      const browser = await playwright[b].launch({headless: headlessFlag, viewport: {width:1920, height:1080}});
      console.log(browser)
...
async function recursiveExploration(page, link, depth, parentState){
  ...
  if(!!body) {
    ...
    if(link.includes(baseUrl)){ 
      console.log("AAAA")//Only explore pages of the specified domain
      let elementList = []; 
