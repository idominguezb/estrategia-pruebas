...
describe('Should login and create a tag with twitter description succesfully', () => {
    before(() => {
        cy.request('https://my.api.mockaroo.com/tag.json?key=81544930').then(
            (json) => {
                ...
                let tag                = tags[Math.floor(Math.random() * tags.length)];
                    ...
                    twitterDescription = tag.meta_description;
            }
        );
    ...
    it('twitter title input should have aleatory value', () => {
        cy.reload();
        tag.toggleSection(2)
            .validateValue('[name="twitterDescription"]', twitterDescription)
            .wait(3000);
    });
});
