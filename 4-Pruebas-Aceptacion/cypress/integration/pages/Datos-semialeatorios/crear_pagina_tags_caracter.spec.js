...
const page=new Page()
describe("Crear pagina y asociar un tag con  solo caracteres no alfanumericos", function () {
    let  pageDatos=[]
    before(function(){
        cy.request("https://api.mockaroo.com/api/c98be0f0?count=1000&key=a8a44f40").then((response)=>pageDatos=response.body)
    })
    it("Crear pagina y asociar un tag con  solo caracteres no alfanumericos", function () {
     login.go()
     cy.wait(2000)
